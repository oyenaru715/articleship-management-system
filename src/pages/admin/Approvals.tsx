...
const AdminApprovals: React.FC<{ navigate?: (page: string) => void }> = ({ navigate }) => {
  ...
  const sidebarItems = [
    ...
    { icon: '✅', label: 'Approvals', active: true, page: 'admin-approvals' },
    ...
    <div className="min-h-screen" style={{ background: '#f0f4ff' }}>
      ...
      <div className="flex">
        ...
        <main className="flex-1 p-6 space-y-6">
          ...
          {/* Header */}
          <div>
            <h2 className="text-3xl font-bold" style={{ color: '#0f1f35' }}>✅ Approval Workflow</h2>
            <p className="text-gray-500 text-sm">Review and process pending leave & comp-off requests</p>
          </div>
                ...
                <div className="space-y-4">
                  ...
                  <div className="bg-white rounded-2xl shadow-md p-6">
                    ...
                    <div className="space-y-3">
                      {[
                        { stage: 'Article', status: selected.articleApproved ? 'Approved' : 'Pending', color: selected.articleApproved ? '#16a34a' : '#d4a017' },
                        { stage: 'Senior', status: selected.seniorStatus, color: selected.seniorStatus === 'Approved' ? '#16a34a' : selected.seniorStatus === 'Rejected' ? '#dc2626' : '#d4a017' },
                        { stage: 'Partner (You)', status: selected.partnerStatus, color: selected.partnerStatus === 'Approved' ? '#16a34a' : selected.partnerStatus === 'Rejected' ? '#dc2626' : '#d4a017' },
                      ].map((item, i) => (
                        <div key={item.stage}>
                  ...
                  {selected.status.includes('Pending') && (
                    <div className="bg-white rounded-2xl shadow-md p-6">
                      <h4 className="font-bold mb-4" style={{ color: '#0f1f35' }}>🎯 Your Action Required</h4>
