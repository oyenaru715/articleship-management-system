...
const PartnerReports: React.FC<{ navigate?: (page: string) => void }> = ({ navigate }) => {
  ...
  const sidebarItems = [
    ...
    { icon: '📈', label: 'Reports', active: true, page: 'partner-reports' },
  ];
      ...
      <div className="flex">
        ...
        <main className="flex-1 p-6 space-y-6">
          ...
          <div className="flex justify-between items-center">
            <div>
              <h2 className="text-3xl font-bold" style={{ color: '#0f1f35' }}>📈 Partner Analytics & Reports</h2>
              <p className="text-gray-500 text-sm">Monitor articles performance and compliance metrics</p>
            </div>
          ...
          <div className="space-y-6">
            ...
            {activeTab === 'overview' && (
              <div className="space-y-6">
                ...
                {/* Key Insights */}
                <div className="bg-white rounded-2xl shadow-md p-6">
                  <h3 className="font-bold mb-4" style={{ color: '#0f1f35' }}>💡 Key Insights</h3>
                  <div className="space-y-3">
                    {[
                      { insight: 'All articles maintaining attendance above 85% target', icon: '✅', color: '#16a34a' },
                      { insight: 'Vikram Singh completing articleship on track', icon: '🎓', color: '#7c3aed' },
                      { insight: '2 articles with excellent compliance (98%+)', icon: '⭐', color: '#d4a017' },
                      { insight: 'Average approval processing time: 1.8 days', icon: '⚡', color: '#0891b2' },
                    ].map((item, i) => (
                      <div key={i} className="flex items-center space-x-3 p-3 rounded-lg" style={{ background: '#f0f4ff' }}>
