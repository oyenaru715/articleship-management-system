...
const SeniorDashboard: React.FC<{ navigate?: (page: string) => void }> = ({ navigate }) => {
  const [activeMetric, setActiveMetric] = useState('all');

  const sidebarItems = [
    { icon: '📊', label: 'Dashboard', active: true, page: 'senior-dashboard' },
    { icon: '👤', label: 'Recommendations', active: false, page: 'senior-recommendations' },
  ];
  ...
  const pendingRecommendations = [
    ...
    { id: 2, article: 'Rohan Patel', type: 'Leave', days: 1, reason: 'Medical appointment', appliedOn: '05 Jun 2026', status: 'Pending - Senior', stage: 1 },
    { id: 3, article: 'Isha Desai', type: 'Comp-Off', days: 1, reason: 'Worked on 2nd Saturday', appliedOn: '04 Jun 2026', status: 'Pending - Senior', stage: 1 },
  ];
  ...
  const stats = [
    ...
    { label: 'Pending Recommendations', value: 3, icon: '⏳', color: '#d4a017', bg: '#fef3c7' },
    { label: 'Recommendations Given', value: 15, icon: '👍', color: '#0891b2', bg: '#cffafe' },
    { label: 'Avg Attendance %', value: '94', icon: '📊', color: '#16a34a', bg: '#dcfce7' },
  ];
    ...
    🚪 Logout
      ...
      <div className="flex">
        ...
        <main className="flex-1 p-6 space-y-6">
          ...
          {/* Header */}
          <div>
            <h2 className="text-3xl font-bold" style={{ color: '#0f1f35' }}>📊 Senior Dashboard</h2>
            <p className="text-gray-500 text-sm">Provide recommendations for leave & comp-off requests before Partner approval</p>
          </div>
          ...
          <div className="rounded-xl p-4 border-l-4" style={{ background: '#fef3c7', borderColor: '#d97706' }}>
            <p className="font-bold text-amber-800">⚠️ 3 Recommendations Pending Your Review</p>
            <p className="text-amber-700 text-sm mt-1">Naresh's Leave, Rohan's Leave, and Isha's Comp-Off are awaiting your recommendation.</p>
          </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <div className="bg-white rounded-2xl shadow-md overflow-hidden">
              <div className="px-6 py-4 border-b border-gray-100" style={{ background: '#f0f4ff' }}>
                <h3 className="font-bold text-lg" style={{ color: '#0f1f35' }}>⏳ Pending My Recommendation (3)</h3>
              </div>
              <div className="divide-y divide-gray-100">
                {pendingRecommendations.map((rec) => (
                  <div key={rec.id} className="p-4 hover:bg-blue-50 transition-all cursor-pointer">
                    <div className="flex items-start justify-between mb-2">
                      <div>
                        <p className="font-medium text-gray-800">{rec.article}</p>
                        <p className="text-xs text-gray-500">{rec.type} • {rec.days} day{rec.days > 1 ? 's' : ''}</p>
                      </div>
          ...
          <div className="bg-white rounded-2xl shadow-md p-6">
            <h3 className="font-bold mb-4" style={{ color: '#0f1f35' }}>👤 Your Role as Senior</h3>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              {[
                {
                  icon: '📋',
                  title: 'Review Requests',
                  desc: 'Evaluate leave and comp-off requests from articles based on their attendance, leaves, and performance.'
                },
                {
                  icon: '✅',
                  title: 'Provide Recommendations',
                  desc: 'Recommend approval or rejection of requests with detailed remarks for Partner\'s final decision.'
                },
                {
                  icon: '📊',
                  title: 'Track Performance',
                  desc: 'Monitor articles\' compliance, attendance trends, and overall articleship progress continuously.'
                }
              ].map((role, i) => (
                <div key={i} className="rounded-xl p-4" style={{ background: '#f0f4ff' }}>
                  <p className="text-2xl mb-2">{role.icon}</p>
                  <p className="font-bold text-gray-800 mb-2">{role.title}</p>
          ...
          <div className="bg-white rounded-2xl shadow-md p-6">
            <h3 className="font-bold mb-4" style={{ color: '#0f1f35' }}>🔄 Recommendation Workflow</h3>
            <div className="flex items-center justify-between">
              {[
                { stage: 'Article Applies', icon: '📝', status: 'Complete' },
                { stage: 'You Recommend', icon: '👤', status: 'Active' },
                { stage: 'Partner Approves', icon: '👨‍💼', status: 'Waiting' },
              ].map((workflow, i) => (
                <div key={i} className="flex flex-col items-center">
                  <div
                    className="w-16 h-16 rounded-full flex items-center justify-center text-2xl mb-2"
                    style={{
                      background: workflow.status === 'Complete' ? '#dcfce7' : workflow.status === 'Active' ? '#fef3c7' : '#f0f4ff',
                    }}
                  >
                    {workflow.icon}
                  </div>
                  <p className="text-sm font-bold text-center" style={{ color: '#1e3a5f' }}>{workflow.stage}</p>
                  <p className="text-xs text-gray-500 mt-1">{workflow.status}</p>
                </div>
              ))}
            </div>
          </div>
